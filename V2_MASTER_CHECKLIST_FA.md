...
- [ ] فایل Release Note نهایی v1 تکمیل شود (فقط خلاصه عملیاتی، بدون تحلیل اضافی).
- [ ] Tag رسمی Backup v1 ایجاد شود.
- [ ] Tag به `origin` push شود.
- [ ] لینک Tag به عنوان مبنای شروع v2 ثبت شود.

### 2.3 معیار پذیرش این فاز
- [ ] با یک دستور `git checkout <tag>` بتوان دقیقا v1 را بازگردانی کرد.
- [ ] تمام اعضا روی یک نقطه مرجع ثابت قبل از v2 هم نظر باشند.
