...
- [ ] سیاست ترافیکی را قطعی کنیم:
  - [ ] `always-on` برای همه queryها
...
- [ ] enforce واقعی `--forward-max-conns-per-ip` در `run_forward_server`.
- [ ] اصلاح `max_inflight_per_resolver` تا روی resolver فعال هم واقعا اعمال شود.
- [ ] اصلاح رفتار race در پردازش `done_set` تا success همزمان از دست نرود.
- [ ] تنظیم قابل کانفیگ برای zombie detection (به‌جای ثابت 4 ثانیه).
- [ ] اضافه‌کردن log ساخت‌یافته برای retry/backoff (metric-ready).

**پذیرش فاز 1**
- [ ] تست‌های فعلی هسته پاس باشند: `48 passed`.
...
- [ ] انتخاب resolverها با اولویت کیفیت واقعی selector (نه random ساده).
- [ ] افزودن پارامترها:
  - [ ] `--broadcast-enable/disable`
  - [ ] `--broadcast-fanout` (پیش‌فرض 10)
  - [ ] `--broadcast-timeout`
  - [ ] `--broadcast-per-resolver-timeout`
- [ ] پیاده‌سازی `first valid response wins` با cancel صحیح losers.
- [ ] اتصال به `ResolverSelector.report_success/failure` برای هر resolver شرکت‌کننده.
- [ ] طراحی استراتژی fallback:
  - [ ] اگر broadcast fail شد -> مسیر serial فعلی
  - [ ] اگر بخشی fail شد -> ادامه با پاسخ winner
...
- [ ] تثبیت idempotency بر پایه `(msg_type, session_id, nonce)`.
- [ ] برای `HELLO` قانون جداگانه اعتبارسنجی تعریف شود:
  - [ ] nonce باید match شود
  - [ ] session_id نباید hardcode/پیش‌فرض فرض شود
- [ ] بررسی دوباره dedup cache سرور برای queryهای broadcast همزمان.
- [ ] بررسی safety در `TYPE_STREAM_SEND` تا side effect تکراری ایجاد نشود.
...
- [ ] افزودن شمارنده‌ها:
  ...
  - [ ] `resolver_switch_total`
- [ ] افزودن latency distribution:
  - [ ] p50 / p95 / p99 query latency
- [ ] افزودن گزارش دوره‌ای KPI در لاگ (هر 60 ثانیه).
...
- [ ] `resolver_max_inflight=1` (تا بعد از KPI اولیه)
- [ ] `resolver_attempt_cap=6` (با بازنگری بعد از KPI)
...
## Work Log
- [ ] 2026-03-12: ایجاد چک‌لیست اجرایی broadcast/multipath. (RmnJL)
